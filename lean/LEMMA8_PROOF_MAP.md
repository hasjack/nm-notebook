...
Layer C spectrum (Hire/Cone.lean):
  proved:
    ...
    eigenvector_of_leaf_mode, lapMatrix_GX_mulVec_leafMode
      ...
      only when ∑ v = 0
  sorry (ordered identification, not a set union):
...
| C | Cone block `[ k -1ᵀ ; -1 I+L' ]` and the ordered shift `λᵢ = 1 + μᵢ` for `i ≥ 2` | Block equality proved, no sorry. Eigenvalues `0` and `n` proved. Leaf mode `eigenvector_of_leaf_mode` proved (`∑ v = 0`). Ordered spectrum `spectrum_lapMatrix_GX` is one sorry; extra eigenvalues start at μ₂ |
...
3. Layer C block equality is proved (`lapMatrix_GX_reindex_eq_fromBlocks`). Eigenvalue `0` (all-ones) and eigenvalue `n` (star-max, gold differences zero) are proved. `eigenvector_of_leaf_mode` is proved when `∑ v = 0`. The ordered spectrum `spectrum_lapMatrix_GX` (`λᵢ = 1 + μᵢ` for `i ≥ 2`) is one sorry. The set form is not stated: it would miss a repeated `μ₂ = 0`. Card 4 is that case, still a comment, not a new proof.
...
- Open: `spectrum_lapMatrix_GX` (one sorry; ordered `λᵢ = 1 + μᵢ` from `μ₂`) and the connectivity step of `lambda2_eq_one_iff_GoldLeavesDisconnected` (second sorry; Layer D not checked). No sorry in `lemma8_converse_card_three`. Dirichlet owner stub in `Dirichlet.lean`
